...
public class StringUtils {
    ...
    private static final Pattern PATTERN_ISURL =
            Pattern.compile("^(http://|https://)?([a-zA-Z0-9-]+\\.)+[a-zA-Z0-9]{1,4}(/.*)?");
    ...
    /**
     * 要把输入内容组合成 sql语句时, 使用该函数过滤特殊字符以防SQL注入攻击 1. 如果期待数字型输入, 使用isNumeric方法判断是否数字
    ...
    public static String encodeBase64(final String str) {
        ...
        return new String(Base64.getEncoder().encode(StringUtils.getBytes(str)));
    }
    ...
    public static String decodeBase64(final String str) {
        ...
        try {
            return StringUtils.toString(Base64.getDecoder().decode(str));
        } catch (Exception e) {
            return str;
        }
    }
...
//		javax.servlet.ServletContext context =  javax.servlet.ServletContext;
     ...
     * @return
     */
    public final static boolean exit(String inv, String reg) {
...
//		}
    ...
    public static boolean exitTel(String inv) {
        return exit(inv, "(\\d{4}|\\d{3})-(\\d{7,8}|\\d{7,8})");
    ...
    public static boolean exitEmail(String inv) {
        return exit(inv, "[\\w\\.-]+@([a-zA-Z0-9-]+\\.)+[a-zA-Z]{1,4}");
    }
...
//		return exit(inv, "([-a-zA-Z0-9]+\\.)+[a-zA-Z0-9]{1,4}");
        return exit(inv, "([-a-zA-Z0-9]+\\.)+(com|net|org|mobi|info|biz|cc|tv|asia|me|travel|tel|name|co|so|cn|gov|hk){1}");
...
//		String reg = "(?i)<embed (.+?)src=(.+?.(\\.swf))(.+?)>";
    ...
    public static long percentTotal(long nowNum, long total) {
        ...
        if (nowNum <= 0 || total <= 0) return 0;
        if (nowNum == total) return 100;
        Double.valueOf(nowNum);
// 		return (nowNum*100/total);
    ...
    public static boolean isBetweenTime(String now, String start, String end) {
        ...
        Date nowD = DateUtils.StrToDate(now);
        Date startD = DateUtils.StrToDate(start);
        Date endD = DateUtils.StrToDate(end);
        if (nowD == null || startD == null || endD == null) return false;
