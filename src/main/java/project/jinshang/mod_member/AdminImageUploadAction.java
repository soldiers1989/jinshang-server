...
@RestController
@RequestMapping("/rest/admin/image")
@Api(tags = "后台图片库上传模块",description = "后台图片上传接口")
@SessionAttributes(AppConstant.ADMIN_SESSION_NAME)
public class AdminImageUploadAction {
    private static Logger logger = LoggerFactory.getLogger(AdminImageUploadAction.class);
    ...
    @PreAuthorize("hasAuthority('" + AdminAuthorityConst.PHOTOSTORE + "')")
    public OneImageRet watermark(@RequestParam(value = "file", required = true) CommonsMultipartFile file,@RequestParam(value = "flag", required = true) boolean flag,@RequestParam(value = "imagegroupid", required = true) long imagegroupid,Model model) throws IOException, ParseException {
        ...
        if(!imageGroup.getMemberid().equals(admin.getId())){
            oneImageRet.setResult(BasicRet.ERR);
        ...
        if(file !=null && file.getSize()>0) {
          ...
          String ossurl =  ossManageUtil.uploadImage(file,flag,admin.getId(),nowDate);
          oneImageRet.data.setOssurl(ossurl);

          ImageRecord imageRecord = new ImageRecord();
          imageRecord.setMemberid(admin.getId());
    ...
    @PreAuthorize("hasAuthority('" + AdminAuthorityConst.PHOTOSTORE + "')")
    public MoreImageRet morewatermark(@RequestParam(value = "files", required = true)List<CommonsMultipartFile> files,@RequestParam(value = "flag", required = true) boolean flag,@RequestParam(value = "imagegroupid", required = true) long imagegroupid,Model model) throws IOException, ParseException {
        Admin admin = (Admin) model.asMap().get(AppConstant.ADMIN_SESSION_NAME);
            ...
            for (CommonsMultipartFile file : files) {
                ...
                try {
                    ...
                    if(flag) {
                        ...
                        ossurl = ossManageUtil.uploadFileImage(fileContent, "images/data"+"/"+admin.getId(), fileName,admin.getId(),nowDate);
                    }
                    else{
                        // 流转换 转换为oss所需的InputStream
    ...
    public BasicRet insertImageGroupInfo(ImageGroup imageGroup, Model model) {
        ...
        criteria.andGroupnameEqualTo(imageGroup.getGroupname()).andStatusEqualTo(Quantity.STATE_1).andMemberidEqualTo(admin.getId());
        ...
        imageGroup.setMemberid(admin.getId());
    ...
    public BasicRet updateImageGroupInfo(ImageGroup imageGroup,Model model) {
        ...
        }else if(!oldImageGroup.getMemberid().equals(admin.getId())){
            basicRet.setResult(BasicRet.ERR);
    ...
    public BasicRet deleteImageGroupInfo(long id,Model model) {
        ...
        List<ImageRecord> imageRecordList = imageRecordService.selectByGroupIdAndStatus(id,admin.getId());
        if (oldImageGroup == null) {
            basicRet.setResult(BasicRet.ERR);
            basicRet.setMessage("不存在该分组");
        }else if(!oldImageGroup.getMemberid().equals(admin.getId())){
            ...
            List<ImageRecord> list = imageRecordService.selectByGroupIdAndStatus(id,admin.getId());
    ...
    public BasicRet getdetail(Long id,Model model) {
        ...
        if (imageGroup == null) {
            basicRet.setResult(BasicRet.ERR);
            basicRet.setMessage("不存在该分组");
        } else if(!imageGroup.getMemberid().equals(admin.getId())){
            basicRet.setResult(BasicRet.ERR);
    ...
    public BasicRet deleteImageRecordInfo(long id,Model model) {
        ...
        }else if(!oldImageRecord.getMemberid().equals(admin.getId())){
            basicRet.setResult(BasicRet.ERR);
    ...
    public BasicRet deleteMoreImageRecord(Long[] ids,Model model) {
        BasicRet basicRet = new BasicRet();
        Admin admin = (Admin) model.asMap().get(AppConstant.ADMIN_SESSION_NAME);
        List<ImageRecord> list = imageRecordService.selectByIdsAndMemberid(Arrays.asList(ids),admin.getId());
        for (ImageRecord imageRecord: list) {
            ImageRecord updateImageRecord = new ImageRecord();
            updateImageRecord.setId(imageRecord.getId());
            updateImageRecord.setStatus(Quantity.STATE_2);
