...
public class OtherProdProvider {
    ...
    /*public  String listOtherProductForSellerCenter(@Param("queryDto")OtherProductQueryDto queryDto){
        StringBuilder sql = new StringBuilder();


        sql.append("select DISTINCT P.* from productinfo P join productStore S on P.id=S.pdid ,Member M,sellercompanyinfo SC where P.memberid=M.id and M.id=SC.memberid and  producttype='其他' and pdstate != 6 ");
        if(queryDto.getMemberid() != null && queryDto.getMemberid()>0){
            sql.append(" and P.memberid=#{queryDto.memberid} ");
        }
    ...
    public  String listOtherProdForAdminExcel(@Param("queryDto")OtherProductQueryDto queryDto){
        ...
        sql.append("select DISTINCT P.*,M.username,SC.companyname,SC.shopname from productinfo P,Member M,sellercompanyinfo SC,productstore s where P.memberid=M.id and M.id=SC.memberid and p.id=s.pdid and producttype='其他' and pdstate != 6 ");
        if(queryDto.getMemberid() != null && queryDto.getMemberid()>0){
            sql.append(" and P.memberid=#{queryDto.memberid} ");
        }
