 ...
 *
 * @author xiazy
 * @create 2018-07-03 19:21
 **/
@RestController
@RequestMapping("/rest/admin/shippingtemplates")
@Api(tags = "管理员运费模版设置")
@SessionAttributes({AppConstant.ADMIN_SESSION_NAME})
public class ShippingTemplatesAdminAction{
        @Autowired
...
//[{"province":"山东,浙江","city":"烟台，青岛，杭州"，"perkilogramcost":1,"defaultfreight":3,"defaultcost":4,"perkilogramadded":2},{"province":"北京,浙江","city":"北京，绍兴"，"perkilogramcost":2,"defaultfreight":5,"defaultcost":4,"perkilogramadded":2}]
            ...
            //保存日志
            memberLogOperator.saveMemberLog(null,admin,"添加运费模版："+temname,request,memberOperateLogService);
...
//            }
            ...
            //保存日志
            memberLogOperator.saveMemberLog(null,admin,"修改运费模版："+temname,request,memberOperateLogService);
...
//            }
            ...
            //保存日志
            memberLogOperator.saveMemberLog(null,admin,"删除运费模版："+templates.getTemname(),request,memberOperateLogService);
        ...
        @ApiOperation("模版详情")
        @PreAuthorize("hasAuthority('"+ AdminAuthorityConst.ORDERCOLLECTIONS+"')")
        public TemplateRet detail(@RequestParam(required = true) long shopgroupid, Model model){
            TemplateRet templateRet = new TemplateRet();

//            Member member = (Member) model.asMap().get(AppConstant.MEMBER_SESSION_NAME);
            Admin admin = (Admin) model.asMap().get(AppConstant.ADMIN_SESSION_NAME);
            ShippingTemplates templates = shippingTemplatesService.getByShopgroupid(shopgroupid);
            if(templates == null){
...
//            Member member = (Member) model.asMap().get(AppConstant.MEMBER_SESSION_NAME);
            ...
            criteria.andShopgroupidNotEqualTo(-1L);
            List<ShippingTemplates> list =  shippingTemplatesService.listTemplates(example);
